...
import { CBadge, CNavGroup, CNavItem, CNavLink, CSidebarNav } from '@coreui/react-pro/src/index'
  ...
  const navLink = (name: string | JSX.Element, icon: string | ReactNode, badge?: Badge) => {
    return (
      <>
        ...
        {name && name}
        {badge &&
          Array.from(Array.isArray(badge) ? badge : [badge], (_badge, index) => (
            <CBadge color={_badge.color} className={_badge.className ?? 'ms-auto'} key={index}>
              {_badge.text}
            </CBadge>
          ))}
      </>
